 ...
 */
public class DispatcherServletERROR {
    ...
    static {
        try {
            ...
            baseDir = new File(
                    DispatcherServletERROR.class.getClassLoader().getResource(".").toURI()
    ...
    public void service(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            /**
             * 不可以直接定位UserController的類對象，因為不依定每次生成的控制器都叫做UserController
             */
            ...
            for (File file : files) {
                ...
                if (cls.isAnnotationPresent(Controller.class)) {//如果是
                    //如果是，那就實例化這個對象並獲取該類中所有公開方法

                    /**
                     * 不必在此實例化對象，因為類中方法未必依訂有被標記的方法(無用聲明)
                     */
...
//                            System.out.println(value);//"/regUser"、"/loginUser"
                            ...
                            if (value.equals(path)) {
                                //調用對應的業務
                                System.out.println("成功理解為業務方法非靜態路徑");
